...
There are 2 Tables in the database:  One for user login/register (username, password) and one for adding cocktails to users favorites.  The tables represent a One-to-Many relationship.  A single user can have many cocktails on their favorites page.  Crow's Foot notation is provided to show the relationship (Inputting birthday was added after Crow's Foot Notation was completed).
